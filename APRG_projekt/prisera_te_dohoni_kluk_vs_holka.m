function varargout = prisera_te_dohoni_kluk_vs_holka(varargin)
% PRISERA_TE_DOHONI_KLUK_VS_HOLKA MATLAB code for prisera_te_dohoni_kluk_vs_holka.fig
%      PRISERA_TE_DOHONI_KLUK_VS_HOLKA, by itself, creates a new PRISERA_TE_DOHONI_KLUK_VS_HOLKA or raises the existing
%      singleton*.
%
%      H = PRISERA_TE_DOHONI_KLUK_VS_HOLKA returns the handle to a new PRISERA_TE_DOHONI_KLUK_VS_HOLKA or the handle to
%      the existing singleton*.
%
%      PRISERA_TE_DOHONI_KLUK_VS_HOLKA('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in PRISERA_TE_DOHONI_KLUK_VS_HOLKA.M with the given input arguments.
%
%      PRISERA_TE_DOHONI_KLUK_VS_HOLKA('Property','Value',...) creates a new PRISERA_TE_DOHONI_KLUK_VS_HOLKA or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before prisera_te_dohoni_kluk_vs_holka_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to prisera_te_dohoni_kluk_vs_holka_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help prisera_te_dohoni_kluk_vs_holka

% Last Modified by GUIDE v2.5 23-Apr-2016 15:05:27

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @prisera_te_dohoni_kluk_vs_holka_OpeningFcn, ...
                   'gui_OutputFcn',  @prisera_te_dohoni_kluk_vs_holka_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before prisera_te_dohoni_kluk_vs_holka is made visible.
function prisera_te_dohoni_kluk_vs_holka_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to prisera_te_dohoni_kluk_vs_holka (see VARARGIN)

% Choose default command line output for prisera_te_dohoni_kluk_vs_holka
...
% UIWAIT makes prisera_te_dohoni_kluk_vs_holka wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = prisera_te_dohoni_kluk_vs_holka_OutputFcn(hObject, eventdata, handles) 
...
if handles.hractahne == 1
    handles.hractahne = 0;  %Aby se støídali
    handles.hractahne2 = 1; %hráèi ve høe
    x = 1; 
    ...
    elseif handles.aktualni_tah == 51 %Když se splní tato podmínka, tak hráè se dostává na políèko domeèku
        ...
        disp(['Vyhrál kluk se ', num2str(handles.zivoty),' životy.']);
        else
        disp(['Vyhrál kluk s ', num2str(handles.zivoty),' životem.']);    
        end
    end
   ...
   if handles.aktualni_tah == handles.hodpredtim_prisery %Podmínka pro vyhazování, když stoupne hráè na pøíšeru
       ...
       elseif handles.zivoty == 0
           ...
           handles.konec = 1;
           if handles.zivoty2 > 1 %Podmínka pro správný pravopis na konci 
             disp(['Vyhrála holka se ', num2str(handles.zivoty2),' životy.']);
           else
             disp(['Vyhrála holka s ', num2str(handles.zivoty2),' životem.']);    
           end
       end
...
elseif handles.hractahne2 == 1 %Tahne druhý hráè
    handles.hractahne = 1; %pøíští tah pojede zase hráè jedna
...
if handles.pocet_tahu > 3 && handles.pocet_tahu2 > 3; %Podmínka, že když hráè tahne tøikrát, až pak vyjede pøíšera
