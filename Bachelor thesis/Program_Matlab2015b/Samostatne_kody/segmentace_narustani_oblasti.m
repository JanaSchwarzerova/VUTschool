...
%                                                    (Vyber jen jedny data)   
   %Naètení umìle vytvoøených dat:      
     ...
     I = imread('obr_narustani_oblasti1.png');     %Obrázek použitý u narùstání oblastí
                                                  %k tomuto obrázku je
...
%Nastavení souøadnic defualtnì:
% rozmery = size(Vstup_obraz);
% x=round(rozmery(1)/2);
% y=round(rozmery(2)/2);

reg_max=0.2; %maximální interval intenzity 
